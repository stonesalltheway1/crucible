...
import { finalizeTestReport, newTestReport, TIER_VALUES, } from "./schema.js";
...
function parseArgs(argv) {
    ...
    if (tier === null) {
        throw new Error("missing --tier=<tier_0_mutation|tier_1_pbt|tier_2_contract|tier_3_proof|tier_4_honest_ci>");
    }
    return { tier };
}
function printHelp() {
...
function emitError(tier, msg) {
    ...
    const report = finalizeTestReport(newTestReport({
        task_id: "",
        diff_hash: "",
...
export async function dispatch(tier, req) {
    switch (tier) {
...
export async function main(argv) {
    ...
    try {
        ...
        process.stderr.write(`crucible-verify-typescript: ${err.message}\n`);
        return 1;
    }
    let raw;
    try {
        ...
        process.stderr.write(`crucible-verify-typescript: stdin read failed: ${err.message}\n`);
        return 1;
    }
    if (raw.trim() === "") {
        process.stderr.write("crucible-verify-typescript: empty stdin — expected VerificationRequest JSON\n");
        return 1;
    }
    let req;
    try {
        ...
        if (err instanceof LeakageError) {
            process.stderr.write(`crucible-verify-typescript: REFUSING — ${err.message}\n`);
            return 2;
        }
        process.stderr.write(`crucible-verify-typescript: invalid request JSON: ${err.message}\n`);
        return 1;
    }
    let report;
    try {
        ...
        emitError(parsed.tier, `dispatch crashed: ${err.message}`);
...
if (isMain) {
    void main(process.argv).then((code) => {
        ...
        process.stderr.write(`crucible-verify-typescript: fatal: ${err?.stack ?? String(err)}\n`);
        process.exit(1);
    });
}
