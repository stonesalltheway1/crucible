...
function makeStrykerConfig(mutateFiles) {
    return {
...
// ---- Subprocess driver --------------------------------------------------
function runStryker(bin, cwd, configPath, wallClockSeconds) {
...
// ---- Public entrypoint --------------------------------------------------
export async function runTier0Mutation(req, opts = {}) {
    ...
    if (preSupplied === undefined) {
        ...
        try {
            ...
            return finalizeTestReport({
                ...
                verdict: "tool_unavailable",
                passed: false,
                error: `cannot write stryker.conf.json: ${err.message}`,
            }, new Date());
        }
        try {
            const result = await runStryker(opts.strykerBin ?? "stryker", cwd, configPath, wallClockSeconds);
            ...
            return finalizeTestReport({
                ...
                verdict: "tool_unavailable",
                passed: false,
                error: `stryker spawn failed: ${err.message}`,
            }, new Date());
        }
    }
    // Parse the report.
    ...
    if (preSupplied !== undefined) {
        ...
        try {
            ...
            return finalizeTestReport({
                ...
                verdict: "failed",
                passed: false,
                error: `cannot read mutation report at ${REPORT_PATH}: ${err.message}\nstryker stderr: ${strykerStderr.slice(-1000)}`,
            }, new Date());
        }
    }
    let parsed;
    try {
        ...
        return finalizeTestReport({
            ...
            verdict: "failed",
            passed: false,
            error: `mutation.json not valid JSON: ${err.message}`,
        }, new Date());
    }
    ...
    const findings = stats.survivors.slice(0, 50).map((s) => ({
...
//# sourceMappingURL=tier0Mutation.js.map