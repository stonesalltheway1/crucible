...
const COUNTEREXAMPLE_RE = /Counterexample:\s*(?<shrunk>[\s\S]*?)(?:\nShrunk\s+(?<shrinks>\d+)\s+time\(s\))?\nGot(?:\s+error)?:\s*(?<got>[\s\S]*?)(?:\nseed:\s*(?<seed>-?\d+))?(?:\n|$)/i;
export function parseCounterexample(property, failureMessage) {
...
// ---- File discovery ------------------------------------------------------
async function findPbtFiles(cwd, candidates) {
    const out = [];
...
// ---- Subprocess driver ---------------------------------------------------
function runVitest(bin, cwd, files, reportPath, numRuns, wallClockSeconds) {
...
// ---- Public entrypoint ---------------------------------------------------
export async function runTier1Pbt(req, opts = {}) {
    ...
    const candidates = opts.testFilesOverride !== undefined
        ? Array.from(opts.testFilesOverride)
        : cls.tests.map((f) => f.path);
    const pbtFiles = opts.preSuppliedReport !== undefined
        ? candidates
    ...
    if (pbtFiles.length === 0 && opts.preSuppliedReport === undefined) {
        const finding = {
            ...
            severity: "error",
            detail: "No property-based test files found in diff. Tier 1 requires at least one *.test.ts that imports fast-check.",
        };
    ...
    if (opts.preSuppliedReport !== undefined) {
        ...
        try {
            const result = await runVitest(opts.vitestBin ?? "vitest", cwd, pbtFiles, reportPath, numRuns, wallClockSeconds);
            try {
            ...
            return finalizeTestReport({
                ...
                passed: false,
                error: `vitest spawn failed: ${err.message}`,
            }, new Date());
        }
        finally {
            // Best-effort cleanup.
    ...
    try {
        ...
        return finalizeTestReport({
            ...
            verdict: "failed",
            passed: false,
            error: `vitest JSON report unparsable: ${err.message}`,
        }, new Date());
    }
    ...
    const passed = report.numFailedTests === 0 &&
    ...
    const findings = counterexamples.slice(0, 50).map((c) => ({
...
//# sourceMappingURL=tier1Pbt.js.map