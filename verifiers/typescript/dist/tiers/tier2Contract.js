...
export function parseJUnitFailures(xml) {
    ...
    while ((m = caseRe.exec(xml)) !== null) {
        ...
        if (!inner)
            continue;
        const failureRe = /<failure\b([^>]*)>([\s\S]*?)<\/failure>|<failure\b([^/]*)\/>/g;
        let f;
...
function extractEndpoint(name) {
    const m1 = /\b(GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS)\s+(\/[^\s\]]*)/i.exec(name);
    if (m1) {
...
// ---- Subprocess driver --------------------------------------------------
function runSchemathesis(argv, cwd, wallClockSeconds) {
...
// ---- Public entrypoint --------------------------------------------------
export async function runTier2Contract(req, opts = {}) {
    ...
    if (opts.preSuppliedJunit !== undefined) {
        ...
        const cmd = opts.schemathesisCmd ??
        ...
        try {
            ...
            return finalizeTestReport({
                ...
                passed: false,
                contract: { spec_path: spec },
                error: `schemathesis spawn failed: ${err.message}`,
            }, new Date());
        }
        finally {
    ...
    const findings = violations.slice(0, 50).map((v) => ({
...
//# sourceMappingURL=tier2Contract.js.map