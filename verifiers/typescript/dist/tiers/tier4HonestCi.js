...
// ---- Subprocess driver --------------------------------------------------
function runPnpm(bin, args, cwd, wallClockSeconds) {
...
// ---- Public entrypoint --------------------------------------------------
export async function runTier4HonestCi(req, opts = {}) {
    ...
    try {
        ...
        const install1 = await runPnpm(pnpm, ["install", "--frozen-lockfile", `--store-dir=${firstStaging}`], cwd, halfBudget);
        ...
        // Second pass.
        const install2 = await runPnpm(pnpm, ["install", "--frozen-lockfile", `--store-dir=${secondStaging}`], cwd, halfBudget);
        ...
        return finalizeTestReport({
            ...
            verdict: "tool_unavailable",
            passed: false,
            error: `tier 4 driver failed: ${err.message}`,
        }, new Date());
    }
    finally {
...
//# sourceMappingURL=tier4HonestCi.js.map