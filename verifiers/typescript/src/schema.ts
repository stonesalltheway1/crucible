...
export const PREDICATE_TYPE = "https://crucible.dev/TestReport/v1" as const;

// ---- Tier enum (matches Go testreport.Tier) -----------------------------
...
// ---- Language (matches Go testreport.Language) --------------------------
...
// ---- Verdict (matches Go testreport.Verdict) ----------------------------
...
export interface MutationStats {
  ...
  timeout?: number;
  total: number;
  score: number; // killed / (killed + survived)
  threshold: number;
  diff_scoped: boolean; // MUST be true in Crucible
...
export interface PBTStats {
  iterations: number;
  iterations_min: number; // Crucible mandate: >=10_000
  properties?: string[];
...
export interface Finding {
  category: string; // e.g. "mutation_survived", "property_failed"
...
export interface TestReport {
  ...
  verdict: Verdict;
  passed: boolean;
  started_at: string; // RFC3339, matches Go time.Time JSON encoding
  finished_at: string;
...
// ---- VerificationRequest (matches Go verification.VerificationRequest) --
...
export function finalizeTestReport(
  report: TestReport,
  finished_at: Date,
): TestReport {
